...
public class Piece {
	...
	*/
	private static TPoint [] nextRotation(TPoint [] points,int Height) {
		TPoint [] newPoints = new TPoint[points.length];
		for(int i = 0;i < points.length;++i){
			TPoint point = points[i];
			int newX = Height - point.y - 1;
			int newY = point.x;
	...
	private int myWidth(TPoint[] points){
		int max = -1;
		for(int i = 0;i < points.length;++i){
			int newX = points[i].x;
			if(max < newX)max = newX;
		}
		return max + 1;
	}

	private int myheight(TPoint[] points){
		int max = -1;
		for(int i = 0;i < points.length;++i){
			int newY = points[i].y;
			if(max < newY)max = newY;
		}
		return max + 1;
	}

	private  void skirtLoad(TPoint[] points){
		skirt = new int[width];
		for(int i = 0;i < width;++i){
			int min = height;
			for(int j = 0;j < points.length;++j){
				TPoint curr = points[j];
				if(curr.x == i && curr.y < min){
					min = curr.y;
				}
			}
			skirt[i] = min;
	...
	*/
	public boolean equals(Object obj) {
		...
		if(this.getBody().length != other.getBody().length)return false;
		int size = getBody().length;
		int [] [] massive = new int [size][size];
		for(int i = 0;i < size;++i){
			massive[this.getBody()[i].y][this.getBody()[i].x]++;
		}
		for(int i = 0;i < size;++i){
			massive[other.getBody()[i].y][other.getBody()[i].x]--;
			if(massive[other.getBody()[i].y][other.getBody()[i].x] != 0)return false;
		}
		return true;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curr = root;
		while(true){
			if(curr.computeNextRotation().equals(root)){
				curr.next = root;
				break;
			}
			curr.next = new Piece(nextRotation(curr.body,curr.height));
			curr = curr.next;
